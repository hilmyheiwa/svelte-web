    ...
    <div class="row">
        ...
        <div class="col-lg-4 col-md-6 col-12 mt-4 pt-2">
            <div class="job-post rounded shadow bg-white">
                <div class="p-4">
                    <Link to={`/job-detail-two/${job.id}`} class="text-dark title h5">{
                        job.title }</Link>

                    <p class="text-muted d-flex align-items-center small mt-3">
                        <i data-feather="clock" class="fea icon-sm text-primary me-1"></i>
                        { job.time }
                    </p>

                    <ul class="list-unstyled d-flex justify-content-between align-items-center mb-0 mt-3">
                        <li class="list-inline-item">
                            <span class="badge bg-soft-primary">{ job.type }</span>
                        </li>
                        <li class="list-inline-item">
                            <span class="text-muted d-flex align-items-center small">
                                <i data-feather="dollar-sign" class="fea icon-sm text-primary me-1"></i>
                                { job.salary }
                            </span>
                        </li>
                    </ul>
                ...
                <div class="d-flex align-items-center p-4 border-top">
                    ...
                    <div class="ms-3">
                        <Link to={`/employer-profile/${job.id}`}
                            class="h5 company text-dark">{ job.name }</Link>
